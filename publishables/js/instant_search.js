function instant_search_results(result) {
    ...
    if (result.items.length) {
        for (i = 0; i < result.items.length; ++i) {
            ...
            list = list.concat('<div><a href="panel/Publisher/pages/edit/' + result.items[i].id + '">' + result.items[i].title);
...
(function ($) {
    $('#instant-search input').keyup(function () {
        ...
        setDelay(function () {
            if (data.length > 2) {
                let formData = 'ajax_action=jsonSearch&ajax_object=InstantSearch&callback=' + callback + '&keyword=' + data + '&type=' + tag;
